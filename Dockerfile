...
# Using the pre-downloaded bcl2fast binary
COPY ./bin/bcl2fastq2-v${BCL2FASTQ_VERSION_MAJOR}-${BCL2FASTQ_VERSION_MINOR}-${BCL2FASTQ_VERSION_REVISION}-linux-x86-64.zip /tmp/
RUN cd /tmp/ && \
...
# directly downloading the bcl2fast binary from Illumina
# RUN cd /tmp/ && \
